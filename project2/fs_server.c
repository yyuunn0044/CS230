...
int main(int argc, char **argv)
{
	...
	printf("============Server Start\n");
	ssock=socket(AF_INET, SOCK_STREAM, 0); //make socket
	if ( ssock<0 ){
		printf("============Socket Error\n");
		exit(0);
	
	}
	memset(&serveraddr, 0, sizeof(serveraddr));       //assignments to characters of serveraddr
	serveraddr.sin_family = AF_INET;
	serveraddr.sin_addr.s_addr = htonl(INADDR_ANY);
	serveraddr.sin_port = htons(20003);

	state = bind(ssock, (struct sockaddr *)&serveraddr, sizeof(serveraddr));
